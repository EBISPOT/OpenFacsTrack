...
class ClinicalSampleFile():
    ...
    def __init__(self, filepath):
        ...
        self.static_columns = ['batch', 'X1', 'Operator name', 'Comments',
                          'Date', 'Panel', 'Clinical_sample']

        # Compute names of parameters present. These are all the other
        # columns in the file that are not in the static_columns list
        parameter_columns = set(self.df.columns) \
            - set(self.static_columns)
    ...
    def validate(self):
        ...
        if len(static_columns_missing) > 0:
            validation_errors['static_columns_missing'] \
                = static_columns_missing
        ...
        if 'Panel' in self.df.columns:
            ...
            for panel in panels_in_data:
                try:
                    panels_in_data_pk.append(
                        Panel.objects.get(name=panel.upper()).id)
        ...
        for parameter_column in self.parameter_columns:
            try:
                parameter_object = Parameter.objects.get(gating_hierarchy=parameter_column, panel__in=panels_in_data_pk)
        ...
        with transaction.atomic():
            ...
            for index, row in self.df.iterrows():
                ...
                for parameter,parameter_pk in parameters_pk.items():
                    if isinstance(row[parameter], numbers.Number) and \
                      not np.isnan(row[parameter]):
                    ...
                    # DEBUG
                        ...
                        message = f"Value ({row[parameter]}) not a " + \
