...
class UploadPanelResultTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        fpath = os.path.join(cls.base_dir, 'test_data','population_names_20200413.xlsx')
    ...
    def test_all_expected_columns_present(self):
        ...
        with open(fpath,'rb') as infile:
            uploaded_file = SimpleUploadedFile(fpath, infile.read(), content_type="text/csv")
    ...
    def test_fatal_error_on_missing_required_column(self):
        ...
        with open(fpath,'rb') as infile:
            uploaded_file = SimpleUploadedFile(fpath, infile.read(), content_type="text/csv")
        clinical_sample_file = ClinicalSampleFile(file_name=file_name,
            file_contents=uploaded_file,
            user=self.user,
            gating_strategy=self.gating_strategy)
    ...
    def test_complete_file_uploaded(self):
        ...
        for expected_result in expected_results:
            ...
            self.assertEqual(ProcessedSample.objects.filter(clinical_sample_id=sample_id).count(),1)

        # Results
        self.assertTrue(Result.objects.count(),3)
        for expected_result, result in zip(expected_results,Result.objects.all()):
            self.assertEqual(expected_result['sample_id'], result.processed_sample.clinical_sample_id)
            self.assertEqual(expected_result['filename'], result.data_processing.fcs_file_name)
...
#        validation_report_contains="Key:static_columns_missing, Value:['X1']"
    ...
    def tearDownClass(cls):
        ...
        if retval != 0:
            message = "Test files uploaded have not been deleted. " + \
